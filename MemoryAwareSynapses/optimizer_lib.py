		...
		for group in self.param_groups:
			...
			for p in group['params']:
				...
				if p in reg_params:
					...
					#get the difference
					param_diff = (curr_param_value - init_val).cuda()

					#get the gradient for the penalty term for change in the weights of the parameters
					local_grad = torch.mul(param_diff, 2*self.reg_lambda*omega).cuda()
				...
				if (momentum != 0):
					...
					if nesterov:
						...
						d_p = buf
				#print("dp device is", d_p.get_device())
				#print("group lr is", group['lr'].get_device())
				#print("p data is", p.data.get_device())
				p.data.cuda().add_(-group['lr'], d_p)
				#print ("dp device is", d_p.get_device())
				#print ("group lr is", group['lr'].get_device())
				#print ("p data is", p.data.get_device())
		return loss
